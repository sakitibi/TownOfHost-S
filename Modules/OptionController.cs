...
namespace TownOfHost
{
    static class CustomOptionController
    {
        ...
        public static void begin()
        {
            ...
            var EnableLastImpostor = new PageObject(AdvImpostorRoleOptions, () => $"<color={Utils.getRoleColorCode(CustomRoles.Impostor)}>{getString("EnableLastImpostor")}</color>: {Utils.getOnOff(Options.EnableLastImpostor.GetBool())}", true, () => { });
            var LastImpostorKillCooldown = new PageObject(AdvImpostorRoleOptions, () => $"<color={Utils.getRoleColorCode(CustomRoles.Impostor)}>{getString("LastImpostorKillCooldown")}</color>(s): {Options.LastImpostorKillCooldown.GetFloat()}{main.TextCursor}", true, () => { Options.LastImpostorKillCooldown.UpdateSelection(0); }, (n) => { });
